/* Used by the RISC OS module header generator. */
#define MBEDTLS_VERSION_STRING_CMHG

/* Norcroft accepts neither C99 inline nor the compiler-specific inline
 * spelling in C90 mode. The functions remain static when this is empty. */
#if !defined(inline) && !defined(__cplusplus)
#define inline
#endif

/* The older AMU image's CLib export has no scalar int64_t typedef.  Keep
 * Mbed TLS's millisecond clock type independent of that legacy header. */
#if !defined(MBEDTLS_PLATFORM_MS_TIME_TYPE_MACRO)
#define MBEDTLS_PLATFORM_MS_TIME_TYPE_MACRO __int64
#endif

/* The AMU image's legacy CLib <stdint.h> has no scalar 64-bit typedefs. */
...
/* Norcroft has no <inttypes.h>; use the CLib-compatible format directly. */
#define MBEDTLS_PRINTF_MS_TIME "lld"

/* DDE's TCPIPLibs supplies the BSD/POSIX socket interfaces used by the
 * optional Mbed TLS socket wrapper. */
...
/* RISC OS supplies mbedtls_ms_time() through the platform_util.c port below. */
 ...
 */
/* AcornSSL is a client-side module; the server-only entry points are not
 * required and trigger a Norcroft internal compiler failure in 4.1.0. */
 ...
 */
/* RISC OS does not use the portable Unix/Windows timing backend. DTLS is not
 * enabled in this configuration, so the optional timing module is not needed.
 */
...
/* CTR_DRBG options */
//#define MBEDTLS_CTR_DRBG_ENTROPY_LEN               32 /**< Removed in Mbed TLS 4.0; the CTR-DRBG header selects the value. */
