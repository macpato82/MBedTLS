...
/* Norcroft uses the portable C byte-swap implementations below. */
...
static inline uint64_t mbedtls_bswap64(uint64_t x)
{
    ...
    /* Norcroft does not accept C99 ULL integer suffixes.  Splitting the
     * value into two 32-bit halves is also valid on the 32-bit target. */
    return ((uint64_t) MBEDTLS_BSWAP32(lo) << 32) |
           (uint64_t) MBEDTLS_BSWAP32(hi);
}
...
/* Keep 64-bit wire-format conversion small enough for the Norcroft C
 * compiler.  The target has a 32-bit data path, so use two 32-bit halves
 * instead of expanding a large 64-bit expression at every call site. */
static inline void mbedtls_put_uint64_be(void *p, uint64_t x)
{
    uint32_t hi = (uint32_t) (x >> 32);
    uint32_t lo = (uint32_t) x;
    ...
    if (MBEDTLS_IS_BIG_ENDIAN) {
        mbedtls_put_unaligned_uint32(q, hi);
        mbedtls_put_unaligned_uint32(q + 4, lo);
    } else {
        mbedtls_put_unaligned_uint32(q, MBEDTLS_BSWAP32(hi));
        mbedtls_put_unaligned_uint32(q + 4, MBEDTLS_BSWAP32(lo));
    }
}

static inline uint64_t mbedtls_get_uint64_be(const void *p)
{
    const unsigned char *q = (const unsigned char *) p;
    uint32_t hi;
    uint32_t lo;

    hi = mbedtls_get_unaligned_uint32(q);
    lo = mbedtls_get_unaligned_uint32(q + 4);
    if (!MBEDTLS_IS_BIG_ENDIAN) {
        hi = MBEDTLS_BSWAP32(hi);
        lo = MBEDTLS_BSWAP32(lo);
    }
...
#define MBEDTLS_PUT_UINT64_BE(n, data, offset) \
    mbedtls_put_uint64_be((data) + (offset), (uint64_t) (n))
